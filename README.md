...
- **Cross-encoder reranking** — RRF's fused order is not the final answer: the fused candidates are re-scored by Voyage's `rerank-2.5-lite`, a cross-encoder that scores each `(query, chunk)` pair jointly rather than independently (as bi-encoder embeddings and lexical rank both do), producing a substantially more precise final ranking. Results below an empirically-calibrated relevance floor are dropped entirely rather than padded in — calibrated by inspecting the model's score distribution across specific, broad, and deliberately off-topic queries against the ingested corpus (see `backend/evals.py`), since the score isn't a calibrated probability and a fixed cutoff has to be chosen from real score behaviour, not assumed.
